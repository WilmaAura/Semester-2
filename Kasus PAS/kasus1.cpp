...
using namespace std;
    ...
    float upahReguler (int jam, float upah){
        float upahReg = jam * upah; //Rumus upah reguler 
        if (jam >= 30){ //jika lembur lebih dari atau sama dengan 30 jam
            upahReg = upahReg + upahReg * 0.4; //akan mendapat tambahan 40% dari upah reguler
        } else{
            upahReg = upahReg + upahReg * 0.2; //kalaau kurang dari 30 jam, akan mendapat 20% saja
        }
        return upahReg; //mengembalikan nilai atau menyimpan nilai upahReg
    }
